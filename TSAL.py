...
class TSAL:
    def __init__(self, data_name, model_name, input_length, init_ratio, total_num_query_step, num_epoch, batch_size,
        ...
        self.label_first_data()
                # ----------------- Meta-Active Learning additions -----------------
        self.meta_switch = False  # Track if a meta-switch happens
        self.al_methods = ["random", "margin", "core", "badge", "entropy", 'utility']  # Available AL strategies
        self.current_al_idx = 0  # Start with random AL

    def label_first_data(self):
        # Force at least 1 label per class
        self.init_ind_selected = []
        np.random.seed(self.seed)
        for i in range(self.num_class):
            available_indices = np.where(self.y_true_train == i)[0]
            if len(available_indices) > 0:
                chosen_index = np.random.choice(available_indices, size=1, replace=False)
                self.init_ind_selected += chosen_index.tolist()
        
        # Now, add extra random labels if init_ratio demands more
        already_selected = set(self.init_ind_selected)
        remaining_indices = list(set(np.arange(len(self.y_true_train))) - already_selected)
        np.random.shuffle(remaining_indices)
        
        num_first_data = np.maximum(int(self.num_avail * self.init_ratio), 1)
        num_remaining_needed = max(num_first_data - len(self.init_ind_selected), 0)
        self.init_ind_selected += remaining_indices[:num_remaining_needed]
        
        self.init_ind_selected = np.array(self.init_ind_selected)
        np.random.seed()
    
        self.labeled_or_not[self.init_ind_selected] = 1
        print(f"Initial labeling done: {np.sum(self.labeled_or_not)}, guaranteed at least 1 sample per class.")
    ...
    def doAL(self, num_query_ratio=0.005, is_semi_supervised=False,  eta=0.8):
        ...
        for query_step in range(self.total_num_query_step):
            ...
            # ----------- Meta-Active Learning Switch Logic -------------
            if query_step > 1:  # Only after 2 rounds
                last_acc = np.mean(test_acc[-2])
                current_acc = np.mean(test_acc[-1])
                improvement = current_acc - last_acc
        
                if improvement < 0.01:  # Less than 1% improvement
                    # Switch AL method
                    self.current_al_idx = (self.current_al_idx + 1) % len(self.al_methods)
                    self.al_name = self.al_methods[self.current_al_idx]
                    print(f"🧠 Meta-AL Scheduler: Switching AL method to {self.al_name}")
        
                    # 🔻 Reduce LR slightly
                    new_lr = self.model_manager.model.optimizer.learning_rate.numpy() * 0.7
                    self.model_manager.model.optimizer.learning_rate.assign(new_lr)
                    print(f"🔻 New Learning Rate: {new_lr:.6f}")
                    self.meta_switch = True
                else:
                    self.meta_switch = False
            # ------------------------------------------------------------
            ...
            prop_indices = np.where(self.labeled_or_not_propagated == 1)[0].astype(np.int64)
            prop_acc_one = np.sum(self.y[prop_indices] == self.y_true_train[prop_indices]) / np.sum(self.labeled_or_not_propagated)

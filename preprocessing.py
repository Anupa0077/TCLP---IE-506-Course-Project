    ...
    def generate_long_time_series(self):
        try:
            ...
            if self.data_name == "50salads":
                ...
                data_path = '/content/drive/MyDrive/TCLP/datasets/MHEALTHDATASET'
                file_names = sorted(glob.glob(os.path.join(data_path, "*.log")))

                if not file_names:  # If no files found, raise an error
                    raise FileNotFoundError(f"No .log files found in {data_path}. Check your dataset path.")
                ...
                if 'y_long' not in locals():
                    raise ValueError("y_long was never initialized. Check if your dataset files are empty.")
            ...
            np.save(os.path.join("/content/drive/MyDrive/TCLP/datasets/", self.data_name + "_X_long.npy"), X_long)
            np.save(os.path.join("/content/drive/MyDrive/TCLP/datasets/", self.data_name + "_y_long.npy"), y_long)
            np.save(os.path.join("/content/drive/MyDrive/TCLP/datasets/", self.data_name + "_y_seg_long.npy"), y_seg_long)
            np.save(os.path.join("/content/drive/MyDrive/TCLP/datasets/", self.data_name + "_file_boundaries.npy"), file_boundaries)

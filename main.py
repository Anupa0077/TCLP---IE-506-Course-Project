...
init_ratio_dict = {"50salads": 0.00016, "HAPT": 0.00006, "GTEA": 0.0006, "mHealth": 0.005, "Breakfast":0.0006}  # set as the number of segments in each dataset
input_length_dict = {"TCN":{"50salads": 512, "HAPT": 512, "GTEA": 128, "SAMSUNG": 128, "HASC_BDD": 512, "Sleep": 1024, "ECG": 512, "mHealth": 256, "Breakfast":512},
                     "MSTCN":{"50salads": 2048, "HAPT": 1024, "GTEA": 1024, "mHealth": 256, "Breakfast":2048},
                     "SSTCN":{"50salads": 2048, "HAPT": 1024, "GTEA": 1024, "mHealth": 1024, "Breakfast":2048}}  #
num_query_ratio = {"50salads": 0.00075, "HAPT": 0.00025, "GTEA": 0.0066, "SAMSUNG": 0.0005, "HASC_BDD": 0.0005, "Sleep": 0.005, "ECG": 0.0001, "mHealth": 0.0005, "Breakfast":0.0005}  #
...
data_epoch_dict = {"50salads":50, "HAPT": 50, "GTEA": 20, "mHealth": 30, "Breakfast":50}
data_batch_dict = {"TCN":{"50salads": 32, "HAPT": 32, "GTEA": 32, "SAMSUNG": 32, "HASC_BDD": 32, "Sleep": 32, "ECG": 32, "mHealth": 32,  "Breakfast": 32},
                   "MSTCN":{"50salads": 1, "HAPT": 8, "GTEA": 1, "mHealth": 64,  "Breakfast": 1},
...
lr_dict = {"TCN": {"50salads": 0.001, "HAPT": 0.0005, "GTEA": 0.001, "SAMSUNG": 0.001, "HASC_BDD": 0.001,
           ...
           "MSTCN": {"50salads": 0.0005, "HAPT": 0.0005, "GTEA": 0.0001, "mHealth": 0.005, "Breakfast":0.0005},
...
parser.add_argument('--num_query', type=int, default=8, help='number of query')
...
def main():
    ...
    metadata_path = "/content/drive/MyDrive/TCLP/metadata"
    os.makedirs(metadata_path, exist_ok=True)  # ensure folder exists
# Fix: Save as object array to support variable-length inner lists
    result_array = np.array(result, dtype=object)
    
    # Save result based on regularization flag
    if NOPLATREG == 1:
        np.save(os.path.join(metadata_path, f"{NAME}_{MODEL}_no{LP}_{AL}_{ETA}_{TAU}_{TEMP}_{SEED}.npy"), result_array)
    else:
        np.save(os.path.join(metadata_path, f"{NAME}_{MODEL}_{LP}_{AL}_{ETA}_{TAU}_{TEMP}_{SEED}.npy"), result_array)
